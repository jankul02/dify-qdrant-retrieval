...
1. Read `docs/dev/sessions/current.md` (pre-loaded — just acknowledge it).
2. Run `git status` and `git log --oneline -5` via `/run git status` and `/run git log --oneline -5`.
3. Report traffic-light state (🟢/🟡/🔴) as defined in `.continue/rules/session-prompt.md`.
4. Ask for branch choice (new / stay / main); run `sh scripts/session_start.sh YYYY-MM-DD-slug` for a new branch.
5. Write goal + start time to `docs/dev/sessions/current.md` via `/add docs/dev/sessions/current.md`.
